...
async function getRecentServiceOrders(orgId: string) {
...
async function getRecentBudgets(orgId: string) {
...
async function getServiceOrdersInProgress(orgId: string) {
...
async function getTotalRevenue(orgId: string) {
...
async function getRevenueThisMonth(orgId: string) {
  const now = new Date();
...
async function getNetProfit(orgId: string) {
...
async function getProductsCount(orgId: string) {
  return prisma.product.count({ where: { organizationId: orgId } });
}
...
export default async function DashboardPage() {
  ...
  if (!orgId) {
    return <DashboardClient hasOrg={false} />;
  }
  ...
  const [
    ...
    getRecentServiceOrders(orgId),
    getRecentBudgets(orgId),
    getServiceOrdersInProgress(orgId),
    getTotalRevenue(orgId),
    getRevenueThisMonth(orgId),
    getNetProfit(orgId),
  ...
  if (session) {
    if (subscription?.status === "active") {
      ...
      const daysRemaining = differenceInDays(
        addDays(new Date(session.user.createdAt), TRIAL_DAYS),
        new Date(),
      );
      if (daysRemaining > 0) {
  ...
  const visibleStats = stats.filter((s) => allowedRoutes.includes(s.href));
  ...
  if (subscription?.status === "active") {
    ...
    subscriptionMessage =
      daysRemaining <= 7 && daysRemaining > 0
        ? `Plano expira em ${daysRemaining} dia(s)`
        : `Plano ${subscription.plan} ativo`;
    subscriptionType =
      daysRemaining <= 7 && daysRemaining > 0 ? "expired" : "active";
  } else {
    const daysRemaining = differenceInDays(
      addDays(new Date(session?.user?.createdAt || new Date()), TRIAL_DAYS),
      new Date(),
    );
    subscriptionMessage =
      daysRemaining <= 0
        ? "Período de teste expirado"
        : `Teste: ${daysRemaining} dias restantes`;
    subscriptionType = daysRemaining <= 0 ? "expired" : "trial";
  }

  return (
    <DashboardClient hasOrg={true}>
      <div className="space-y-8 p-4 md:p-6">
        ...
        <div
          className={`p-4 rounded-xl border flex items-center gap-4 ${subscriptionType === "active" ? "bg-green-500/10 border-green-500/20" : subscriptionType === "expired" ? "bg-red-500/10 border-red-500/20" : "bg-blue-500/10 border-blue-500/20"}`}
        >
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          {visibleStats.map((stat) => (
            <Link key={stat.title} href={stat.href}>
              <Card className="bg-white/5 border-white/10 hover:border-primary/30 transition-all cursor-pointer group">
                  ...
                  <Link key={order.id} href={`/dashboard/service/${order.id}`}>
                    <div className="flex flex-col sm:flex-row sm:items-center justify-between p-3 rounded-lg bg-white/5 hover:bg-white/10 transition-colors group gap-3">
                      <div className="flex-1 min-w-0">
                        <p className="text-sm font-black text-white truncate group-hover:text-primary uppercase italic tracking-tighter">
